  ...
  return (
    <div className="space-y-4">
      <DashboardCard
        ...
        description="Key metrics across all scenarios"
      >
              ...
              {comparisons.length > 0 && (
                <li>
                  ...
                  <span className="font-medium text-foreground">
                    {
                      comparisons.find(
                        (c) => c.projection.summary.netWorthChangeCents === bestChange,
                      )?.scenario.name
                    }
                  </span>
                  {' with '}
      ...
      {/* Assumption & disclosure layer — what the projection assumed, what it
          excluded, and how to read the ranking. */}
      <DisclosurePanel payload={buildScenarioComparisonDisclosure(comparisons)} />
    </div>
  )
}
